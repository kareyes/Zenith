...
import { validateMovement } from './gameplay';
...
export const move = ({ currentPosition, maze, playerMoves }: GamePlayState) =>
  pipe(
    ...
    Effect.tap(({ newPosition, currentPosition }) => Ref.update(currentPosition, () => newPosition)),
 ...
 pipe(
   ...
   Effect.tap(({position, mazeState}) => {
    if(position.x === mazeState.numRows - 1 && position.y === mazeState.numCols - 1) {
...
const drawMaze = (state: GameState) =>
  Effect.gen(function* () {
    clear()
    const layout = yield* builder;
    console.log(layout.join(''));
    const currentPosition = yield* Ref.get(state.currentPosition);
    console.log("state", currentPosition);
  }).pipe(
    Effect.provideServiceEffect(MazeState, Effect.succeed(state.maze)),
    Effect.provideServiceEffect(CurrentPositionState, Effect.succeed(state.currentPosition)),
...
export const mazeInit = Effect.gen(function* () {
  ...
  listener({ maze, currentPosition }).pipe(
     Effect.runPromise,
  );
});
